...
exports.submitLGBTQProfile = async (req, res) => {
  try {
    ...
    const formStatus = await FormStatus.findOne({ formName: 'LGBTQIA+ Profiling' });
    if (!formStatus || !formStatus.isOpen) {
      return res.status(403).json({ error: 'Form is currently closed' });
    }

    const existing = await LGBTQProfile.findOne({ user: userId, cycleId: formStatus.cycleId });
    if (existing) {
    ...
    res.status(201).json({ message: 'LGBTQIA+ Profile submitted successfully' });
  } catch (error) {
...
exports.getAllProfiles = async (req, res) => {
  try {
    const profiles = await LGBTQProfile.find().populate('user', 'username email');
...
exports.getProfileById = async (req, res) => {
  try {
    const profile = await LGBTQProfile.findById(req.params.id).populate('user', 'username email');
...
exports.getMyProfile = async (req, res) => {
  try {
    ...
    if (!profile) return res.status(404).json({ error: 'No profile found for this cycle' });
...
exports.updateProfileById = async (req, res) => {
  try {
    ...
    if (req.body.sexAssignedAtBirth) profile.sexAssignedAtBirth = req.body.sexAssignedAtBirth;
    if (req.body.lgbtqClassification) profile.lgbtqClassification = req.body.lgbtqClassification;
...
exports.exportProfilesToExcel = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=lgbtq_profiles.xlsx');
